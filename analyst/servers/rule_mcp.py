...
DEFAULT_RULES = [
    {"rule_id": "R001", "name": "injection_detected", "description": "检测到注入攻击（SQL/XSS/命令注入）",
     "action": "block", "severity": "critical", "enabled": True,
     "patterns": ["sql injection", "injection", "xss", "union select", "1=1", "<script>", "../etc/passwd"]},
    {"rule_id": "R002", "name": "sensitive_data_leak", "description": "检测到敏感数据泄露",
     "action": "block", "severity": "critical", "enabled": True,
     "patterns": ["password", "secret", "token", "api_key", "private key", "-----BEGIN"]},
    {"rule_id": "R003", "name": "prompt_injection", "description": "检测到提示词注入攻击",
     "action": "block", "severity": "high", "enabled": True,
     "patterns": ["ignore previous", "system prompt", "you are now", "forget all", "jailbreak"]},
    {"rule_id": "R004", "name": "abnormal_collaboration", "description": "检测到Agent间异常协作（共谋）",
     "action": "block", "severity": "critical", "enabled": True,
     "patterns": ["action_confirmation"],
     "forbidden_pairs": [["tech_support", "refund"]]},
    {"rule_id": "R005", "name": "rate_limit_exceeded", "description": "请求频率异常",
     "action": "alert", "severity": "medium", "enabled": True,
     "patterns": ["rate limit", "too many requests", "429"]},
    {"rule_id": "R006", "name": "data_exfiltration", "description": "潜在数据外泄",
     "action": "block", "severity": "high", "enabled": True,
     "patterns": ["select *", "dump", "export", "download all", "bulk extract"]},
]
